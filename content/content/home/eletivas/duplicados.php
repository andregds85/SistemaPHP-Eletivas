...
<?php          
   ...
   $cmd = "SELECT 
	COUNT(*) AS contador,
	id_fila,numeroSolicitacao,nomePaciente
    FROM
	fila
    GROUP BY
	numeroSolicitacao
  ...
  /*  $cmd = "select * from fila where numeroSolicitacao=numeroSolicitacao ORDER    BY numeroSolicitacao ASC limit $inicio,$registros"; */
    $produtos = mysqli_query($con,$cmd); 
    $total = mysqli_num_rows($produtos); 
    ...
    while ($produto = mysqli_fetch_array($produtos)) { 
     ...
     /* echo "R$ ".$produto['valor']."<br />"; */
      ...
      echo"</td>";    
      echo "<td>";
      echo "<a href='excluiDupli.php?us_id=$produto[id_fila]' class='btn btn-primary'>Apagar</a>";
      echo"</td>";
      echo "</tr>";        
      echo"</tbody>";  
  } 
...
echo "<div class='container'>";
 //exibe a paginação 
    for($i = 1; $i < $numPaginas + 1; $i++) { 
        echo "<a href='duplicados.php?pagina=$i'>".$i."</a> ";  
    } 
echo "</div>";     
echo "</div>";          
echo "</div>";          
